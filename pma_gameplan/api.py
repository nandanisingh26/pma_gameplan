...
def update_task_field(name, field, value):
    ...
    # Optional: add membership validation here

    if field not in ["status", "priority", "progress"]:
...
def get_task_preview(name):
    ...
    # Allow admin
    if frappe.has_role("Gameplan Admin"):
        return task

    # For members → restrict to assigned or same space
    user = frappe.session.user
    ...
    if not frappe.db.exists(
        "PMA Space Member",
        {"parent": task.space, "member": member}
    ):
        frappe.throw("Not allowed")
    ...
    return {
        ...
        "assigned_to_name": frappe.db.get_value(
            "PMA Member",
            task.assigned_to,
            "full_name"
        )
